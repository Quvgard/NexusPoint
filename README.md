...
*   `NexusPoint/`: Основной проект WPF приложения.
    *   `BusinessLogic/`: Содержит классы, инкапсулирующие бизнес-логику (менеджеры, сервисы, калькуляторы).
    *   `Data/`: Содержит классы для взаимодействия с базой данных.
        *   `Repositories/`: Классы-репозитории для каждой сущности (CRUD операции).
        *   `DatabaseHelper.cs`: Утилиты для подключения к SQLite.
        *   `DatabaseInitializer.cs`: Создание таблиц БД.
    *   `Models/`: Содержит классы моделей данных (сущности БД и модели представлений).
    *   `Utils/`: Содержит вспомогательные утилиты.
        *   `Converters/`: WPF конвертеры значений.
        *   `PasswordHasher.cs`: Хеширование паролей.
        *   `PrinterService.cs`: Имитация печати.
    *   `Windows/`: Файлы XAML и Code-behind для окон приложения.
    *   `UserControls/`: (Если есть) Пользовательские элементы управления WPF.
